...
const transaccionesController = {
    ...
    crearSolicitudViaje: async (req, res) => {
        try{
            ...
            const nuevaSolicitud = await SolicitudViaje.create({
                idPasajera, origen, destino, zona, cantPasajeros, estado: 'Pendiente'
            });
    ...
    cancelarSolicitud: async (req, res) => {
        try {
            ...
            return res.json({ message: "Solicitud cancelada correctamente", solicitud });
        } catch (error) {
            return res.status(500).json({ error: error.message });
        }
    },
    ...
    seleccionarConductora: async (req, res) => {
        try {
            ...
            return res.json({ message: "Propuesta enviada a la conductora con éxito", solicitud });
        } catch (error) {
            return res.status(500).json({ error: error.message });
        }
    },
    ...
    responderPropuesta: async (req, res) => {
        try {
            ...
            if (aceptar) {
                ...
                await solicitud.save();
                return res.json({ message: "Propuesta rechazada. Reabierta en el panel de control.", solicitud });
            }
        } catch (error) {
            return res.status(500).json({ error: error.message });
        }
    ...
    registrarViaje: async (req, res) => {
        ...
        try {
            ...
            await t.commit();
            return res.status(201).json({ message: "Viaje registrado e iniciado en ruta", nuevoViaje });
        } catch (error) {
            await t.rollback();
            return res.status(500).json({ error: error.message });
        }
