  ...
  .then(async () => {
    ...
    try {
      ...
      if (!existe) {
        ...
        console.log(
          "✅ ¡Usuaria de prueba (Admin) creada visualmente en la BD!",
        );
      }

      // Pasajera de prueba para el filtro de seguridad
      const existePasajera = await Usuario.findOne({
        where: { nomUsuario: "ana_pasajera" },
      });
      if (!existePasajera) {
        await Usuario.create({
          nombre: "Ana López",
      ...
      const existeConductora = await Usuario.findOne({
        where: { nomUsuario: "valen_chofer" },
      });
      if (!existeConductora) {
        await Usuario.create({
